...
// import { Animatable , stopAnimation } from 'react-native-animatable';
    ...
    const options = Object.keys(currentOptions).map(function (k) {
      return (<View key={k} style={{ margin: 10 }}>
        ...
        {/* <Animbutton
          ...
          onColor={_this.state.colorBtn}
          // onColor = {_this.state.colorBtnState === 0 ? "#bdbdbd" : "Green"}
          _onPress={(status) => _this._answer(status, k)}
          text={currentOptions[k]}

        /> */}

        <TouchableWithoutFeedback onPress={(status) => _this._answer(status, k)}>
          <Animatable.View ref="view"
            style={{
              ...
              paddingRight: 20,
              paddingLeft: 20,
              backgroundColor: _this.state.status ? "green" : "#bdbdbd",
              borderRadius: 20
            }}>
            <Text style={{
              color: _this.state.status ? "white" : "#696969",
              fontWeight: "bold"
            }}>
              {/* {this.props.text} */}
              {currentOptions[k]}
